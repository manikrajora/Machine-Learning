...
plt.ylabel("SSE")
plt.title('Elbow Method for Wine Data Set')
...
kmeans = KMeans(n_clusters=2,random_state=1).fit(sse_ica)
labels =  kmeans.predict(trainX_scaled)
res=kmeans.__dict__
c = ['red','green']
l = ['Cluster 1', 'Cluster 2']
for i in range(2):
    plt.scatter(trainX_scaled[:, 3][labels==i], trainX_scaled[:, 10][labels==i], color=c[i],
             label=l[i]);
plt.xlabel('Feature 3')
plt.ylabel('Feature 10')
plt.legend()
# combined = np.hstack((trainX_scaled,np.reshape(labels,(-1,1))))
# df = pd.DataFrame(data=combined)
# sns.set(style="ticks")

# sns.pairplot(df, hue=11)
plt.show()
...
plt.title('Cluster Mean Values for Each Feature for Wine Data Set')
plt.legend()
plt.show()
...
plt.xlabel('Feature Number')
plt.ylabel('Variance')
plt.title('Cluster Variance Values for Each Feature for Wine Data Set')
plt.legend()
plt.show()
...
# combined2 = np.hstack((X,np.reshape(Y_first,(-1,1))))
# print(Y_first)
plt.hist(Y_first)
plt.show()
print(len(np.where(Y_first==3)[0]))
print(len(np.where(Y_first==4)[0]))
print(len(np.where(Y_first==5)[0]))
print(len(np.where(Y_first==6)[0]))
print(len(np.where(Y_first==7)[0]))
print(len(np.where(Y_first==8)[0]))
print(len(np.where(Y_first==9)[0]))
...
for n_cluster in range(2, 20):
    kmeans = KMeans(n_clusters=n_cluster,random_state=1).fit(trainX_scaled)
    label = kmeans.labels_
    sil_coeff = silhouette_score(trainX_scaled, label, metric='euclidean')
...
plt.title('Silhouette Score for Wine Data Set')
plt.legend()
plt.show()  
...
plt.title('BIC score per model for Wine Data Set')
...
for n_cluster in range(2, 20):
    clf1 = mixture.GaussianMixture(n_components=n_cluster,random_state=4).fit(trainX_scaled)
    label = clf1.predict(trainX_scaled)
    sil_coeff = silhouette_score(trainX_scaled, label, metric='euclidean')
    print("For n_clusters={}, The Silhouette Coefficient is {}".format(n_cluster, sil_coeff))
    sil_all.append(sil_coeff)
...
xplot = np.zeros((r,11))
xplot[:,:] = np.arange(0,11,1)
c=['red','green','blue','yellow','purple', 'black', 'orange', 'pink']
l=['Cluster 1', 'Cluster 2', 'Cluster 3','Cluster 4', 'Cluster 5', 'Cluster 6', 'Cluster 7', 'Cluster 8']
for i in range(r):
...
tmp = pd.Series(data = pca.explained_variance_ratio_ ,index = range(1,12))
tmp2 = pd.Series(data = pca.explained_variance_ ,index = range(1,12))
print(tmp2)
...
pcat = PCA(n_components=11,random_state=1)
t_ = pcat.fit_transform(trainX_S)
...
e_all = []
for i in n_comps_all:
    pcat = GaussianRandomProjection(n_components=i)
    t_ = pcat.fit_transform(trainX_S)
...
n_comps_all = np.arange(1,7,1)
e_all = []
clft  = LinearDiscriminantAnalysis(n_components=11)
...
plt.title('Variance Ratio for Wine Data Set')
plt.show()
...
sse_pca = {}
for k in range(1, 20):
    kmeans = KMeans(n_clusters=k,random_state=1).fit(t_[:,:2])
    #print(data["clusters"])
    sse_pca[k] = kmeans.inertia_ # Inertia: Sum of distances of samples to their closest cluster center
plt.figure()
plt.plot(list(sse_pca.keys()), list(sse_pca.values()),'o')
...
for n_cluster in range(2, 20):
    kmeans = KMeans(n_clusters=n_cluster,random_state=1).fit(t_[:,:2])
    label = kmeans.labels_
    print(label)
    sil_coeff = silhouette_score(t_, label, metric='euclidean')
    print("For n_clusters={}, The Silhouette Coefficient is {}".format(n_cluster, sil_coeff))
...
for n_components in n_components_range:
    ...
    gmm = mixture.GaussianMixture(n_components=n_components,random_state=4)
    gmm.fit(t_[:,:2])
    print(gmm.predict(t_[:,:2]))
    bic.append(gmm.bic(t_[:,:2]))
    if bic[-1] < lowest_bic:
        lowest_bic = bic[-1]
        best_gmm = gmm
...
for n_cluster in range(2, 20):
    clf1 = mixture.GaussianMixture(n_components=n_cluster,random_state=4).fit(t_[:,:2])
    label = clf1.predict(t_[:,:2])
    sil_coeff = silhouette_score(t_[:,:2], label, metric='euclidean')
    print("For n_clusters={}, The Silhouette Coefficient is {}".format(n_cluster, sil_coeff))
    sil_all.append(sil_coeff)
...
e_all = []
for i in n_comps_all:
    clft = LinearDiscriminantAnalysis(n_components=i)
    t_ = clft.fit_transform(trainX_S,trainY_comp)
    p_ = clft.inverse_transform(t_)
    loss_all = np.abs((trainX_S - p_)/trainX_S).mean()
    e_all.append(loss_all)
